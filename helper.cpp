...
int 
isChanged(int x, int y, int weight, int array[NUM][NUM]) //checks if table has changed
	//also updates the new weight as needed 
{
	//takes the entire 2D table as input, along with router to check, with it's corresponding line of weights
	int out = -1;
	if(array[x][y]>weight){
		out = 1;
	}
