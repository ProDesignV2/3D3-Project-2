            ...
            || disappearingFlag == 'E' ||disappearingFlag == 'F' || disappearingFlag == 'G'){
           	...
           	if(DistanceVector.parseType(buf) == "Control") {
                ...
                if (dest == *nodeName) {
                    ...
                    int portNum = getnextNode(xpos, ypos);
                    int bufSize = 1024;
                    
                    //these next lines were an issue with Data sending, and we ran out of time to fix it

                    //struct sockaddr *router = get_address("localhost",portNum)->ai_addr;
                    //inet_ntop(router->sa_family, get_in_addr(router), ipstr, sizeof ipstr);
                    //their_addr_size = sizeof their_addr;
                    //send_all(router_fd, buf, &bufSize, router, their_addr_size);
