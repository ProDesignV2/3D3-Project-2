...
void BellmanFord(struct Graph* graph, int src, string data[]) { 
    ...
    int overallpath[V][V];
    ofstream inFile11;
    inFile11.open("overallpath.txt", ios::app);
...
int getnextNode(int dest,int source){
    char overallpath[M][M];
    int next_node; 
    string line;
    int i=0;
    ifstream inFile13;
    inFile13.open("overallpath.txt");
    while (!inFile13.eof()) {

        getline(inFile13, line); 
        //cout<<line<< endl;
        ...
        overallpath[i][0]=  (char_array[0]);
        overallpath[i][1] = (char_array[1]);
        overallpath[i][2]=  (char_array[2]);
        overallpath[i][3]= (char_array[3]);
        overallpath[i][4]= (char_array[4]);
        overallpath[i][5]= (char_array[5]);
        overallpath[i][6]= (char_array[6]);

        cout<< overallpath[i][2]<<endl;
        i++;
        cout <<i << endl;
    ...
    for(int i=0;i<6; i++){
        for(int j=0; j<6; j++){
            cout << overallpath[i][j];
        }
        cout << endl;
    }
    
    next_node = overallpath[source][dest]-48;
    next_node = next_node + 10000;
    inFile13.close();
    return next_node;
